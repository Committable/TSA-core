...
from utils import custom_deepcopy, is_all_real, convert_result, turn_hex_str_to_decimal_arr, convert_result_to_int
from graphBuilder.x_graph import *
from interpreter.symbolic_var_generator import *
from interpreter.evm_params import Z3_TIMEOUT, CONSTANT_ONES_159, LOOP_LIMIT, GAS_LIMIT, UNSIGNED_BOUND_NUMBER, MAX_MEMORY_SIZE
from errors.jump_target import JumpTargetError
from errors.global_timeout import GlobalTimeoutError
...
class EVMInterpreter:
    def __init__(self, runtime, cname, context):
        ...
        # XGraph for the contract, a function -> a XGraph, if not function detected, the all contract -> a XGraph
        self.x_graph = XGraph(cname, self.runtime.source_map)

        # global coverage info for the contract
        # number of paths, terminated by normal or exception
        self.total_no_of_paths = {"normal": 0, "exception": 0, "loopLimit": 0, "gasLimit": 0}
        self.paths = []  # all paths, e.g. [[block1, block2, ...], [block1, block3, ...]]
        # impossible paths
        ...
        # function visited edges and its times, None for not in function, else in a function
        self.function_visited_edges = None

        # the evm runtime bytecode of the contract. todo: should add evm bytecode Node in XGraph?
        self.evm_bytecode = turn_hex_str_to_decimal_arr(self.runtime.binary)  # contract's bytecode in bytes
        # function name on current visiting, null str for not in a function
        self.current_function = ""
        # the path we're visiting
        self.current_path = []
        # todo: solvers for solver z3 constraints, but it's not used now for efficiency
        self.single_solver = Solver()
        self.global_solver = Solver()
        self.single_solver.set("timeout", Z3_TIMEOUT)
        self.global_solver.set("timeout", Z3_TIMEOUT)
            ...
            if self.runtime.source_map.sig_to_func is not None:
                ...
                for key in self.runtime.source_map.sig_to_func:
                    if eval("0x" + key) == eval("0x" + func_sig):
                        current_func_name = self.runtime.source_map.sig_to_func[key]
                        break
    ...
    def sym_exec(self):
        path_conditions_and_vars = {"path_condition": [], "path_condition_node": [], "branch_flag": []}
        ...
        try:
            ...
            traceback.print_exc()
            self.context.set_err()
            log.mylogger.error("cause error when symbolic execute for %s, err: %s", self.cname, str(err))
        return
    ...
    def _terminate_path(self, kind, function_name, params, start_time, block):
        ...
        log.mylogger.debug("block: " + str(block) + " symbolic execution time: %.6f s" % execution_time)
        log.mylogger.debug("*********************************")
        return
    ...
    def _sym_exec_block(self, params, block, pre_block):
        ...
        if block < 0 or block not in self.runtime.vertices:
            log.mylogger.error("unknown block address %d. Terminating this path ...", block)
            self._terminate_path("exception", function_name, params, start_time, block)
            return

        # TODO: how to implement better loop detection? It's a pay-off between time consuming and coverage
        if current_edge in visited and visited[current_edge] > LOOP_LIMIT and self.runtime.jump_type[block] == "conditional":
            log.mylogger.debug("overcome a number of loop limit. Terminating this path ...")
            self._terminate_path("loopLimit", function_name, params, start_time, block)
            return
        else:
            if self.function_visited_edges is None:  # not in a function
                if current_edge in self.total_visited_edges and self.total_visited_edges[current_edge] > 10:
                    log.mylogger.debug("overcome a number of loop limit. Terminating this path ...")
                    self._terminate_path("loopLimit", function_name, params, start_time, block)
                    return
            else:  # in a function
                if current_edge in self.function_visited_edges and self.function_visited_edges[current_edge] > 10:
                    log.mylogger.debug("overcome a number of loop limit. Terminating this path ...")
                    self._terminate_path("loopLimit", function_name, params, start_time, block)
                    return
        ...
        if params.gas > GAS_LIMIT:
            log.mylogger.debug("run out of gas. Terminating this path ... ")
            self._terminate_path("gasLimit", function_name, params, start_time, block)
            return

        # Execute every instruction, one at a time
        # Exception is caught, it may be a bug, but it should not influence other path
        block_ins = self.runtime.vertices[block].get_instructions()
        try:
            for instr in block_ins:
                self._sym_exec_ins(params, block, instr)
        except JumpTargetError as err:
            log.mylogger.error("jump Target Error: %s, Terminating this path ...", str(err))
            self._terminate_path("exception", function_name, params, start_time, block)
            return
        except GlobalTimeoutError as err:  # global timeout means the analysis should be ended globally, so we raise up
            log.mylogger.error("global timeout: %s, Terminating this path ...", str(err))
            self._terminate_path("exception", function_name, params, start_time, block)
            raise err
        ...
        if self.function_visited_edges is not None:
            if current_edge in self.function_visited_edges:
                updated_count_number = self.function_visited_edges[current_edge] + 1
                self.function_visited_edges.update({current_edge: updated_count_number})
            else:
        ...
        if self.runtime.jump_type[block] == "terminal":
            ...
            if branch_expression is not None:
                # todo: we don't check all conditions of the path or even the branch constraint for time consuming,
                #  instead we get string format of branch condition for simple judgement of impossible path
                ...
                if str_expr != "False":
                    ...
                    new_params.path_conditions_and_vars["branch_flag"].append(True)

                    if selector is not None:
                        self.x_graph.add_constraint_node(new_params.path_conditions_and_vars,
                    ...
                    self.impossible_paths.append(copy.deepcopy(self.current_path).append(left_branch))
                ...
                if str_expr != "True":
                    ...
                    self.x_graph.add_constraint_node(params.path_conditions_and_vars,
                                                     self.runtime.vertices[block].end,
                                                     self.gen.get_path_id(),
                                                     )
                ...
                log.mylogger.error("branch expression of conditional jump is None")
                self._terminate_path("exception", function_name, params, start_time, block)
                return
        else:
    ...
    def _sym_exec_ins(self, params, block, instr):
        ...
        if (start_time - self.context.start) >= global_params.SYM_TIMEOUT:
            raise GlobalTimeoutError("global timeout")
        ...
        log.mylogger.debug("==============================")
        log.mylogger.debug("Start executing: " + instr)
        ...
        if opcode == "STOP":
            ...
            if len(stack) > 1:
                ...
                if is_expr(second):
                    path_conditions_and_vars["path_condition"].append(second != 0)
                    path_conditions_and_vars["branch_flag"].append(True)
                    self.x_graph.add_constraint_node(path_conditions_and_vars,
                                                     global_state["pc"]-1,
                                                     self.gen.get_path_id(),
            ...
            if len(stack) > 2:
                ...
                if is_expr(third):
                    path_conditions_and_vars["path_condition"].append(third != 0)
                    path_conditions_and_vars["branch_flag"].append(True)
                    self.x_graph.add_constraint_node(path_conditions_and_vars,
                                                     global_state["pc"] - 1,
                                                     self.gen.get_path_id(),
        ...
        # todo: review this process
            ...
            if len(stack) > 1:
                ...
                if is_all_real(s0, s1) and s0 + s1 <= len(memory):
                    ...
                    new_var_name = self.gen.gen_sha3_var(str(global_state["pc"] - 1))
                    computed = BitVec(new_var_name, 256)
        ...
        #
            ...
            if len(stack) > 0:
                ...
                new_var_name = self.gen.gen_data_var(start, to_symbolic(start + 31), self.current_function)
            ...
            if len(stack) > 2:
                ...
                raise ValueError('STACK underflow')
        elif opcode == "CODESIZE":  # length of the executing contract's code in bytes
            code_size = len(self.evm_bytecode)
            stack.insert(0, code_size)
        elif opcode == "CODECOPY":  # copy executing contract's bytecode
            if len(stack) > 2:
                global_state["pc"] = global_state["pc"] + 1
        ...
        #
            ...
            if len(stack) > 0:
                ...
                for key in global_state["storage"]:
                    if convert_result_to_int(key - position) == 0:
            ...
            if len(stack) > 1:
                ...
                # add to graph
                self.x_graph.add_sstore_node(opcode, global_state["pc"] - 1, [stored_address, stored_value],
                                             self.gen.get_path_id(), path_conditions_and_vars)
            else:
                raise ValueError('STACK underflow')
        elif opcode == "JUMP":
            if len(stack) > 0:
                target_address = convert_result(stack.pop(0))

                if is_expr(target_address):
                    log.mylogger.error("Target address of JUMP must be an integer: but it is %s", str(target_address))
                    raise JumpTargetError("Target address for jump is symbolic")
                else:
                    if target_address not in self.runtime.vertices:
                        raise JumpTargetError("Target address {0} for jump is not in vertices".format(target_address))
            ...
            if len(stack) > 1:
                target_address = convert_result(stack.pop(0))
                if is_expr(target_address):
                    log.mylogger.error("Target address of JUMPI must be an integer: but it is %s", str(target_address))
                    raise JumpTargetError("Target address for jumpi is symbolic")
                else:
                    if target_address not in self.runtime.vertices:
                        raise JumpTargetError("Target address {0} for jumpi is not in vertices".format(target_address))
                    self.runtime.vertices[block].set_jump_targets(target_address)
                    if target_address not in self.runtime.edges[block]:
                        self.runtime.edges[block].append(target_address)
                    ...
                    self.runtime.vertices[block].set_branch_expression(simplify(branch_expression))
            else:
                raise ValueError('STACK underflow')
        elif opcode == "PC":
        ...
        #
            ...
            if len(stack) > 6:
                ...
                for key in global_state["balance"]:
                    if convert_result_to_int(key - recipient) == 0:
                ...
                params.path_conditions_and_vars["path_condition"].append(is_enough_fund)
                ...
                new_var = BitVec(new_var_name, 256)
                stack.insert(0, new_var)
                return_node = ReturnStatusNode(new_var_name, new_var_name, calls[-1])

                # add call instruction to graph
                self.x_graph.add_message_call_node(opcode,
                                                   global_state["pc"] - 1,
            ...
            if len(stack) > 6:
                ...
                old_balance = None
                for key in global_state["balance"]:
                    if convert_result_to_int(key - recipient):
                        old_balance = global_state["balance"].pop(key)
                        break
            ...
            if len(stack) > 5:
                ...
                new_var = BitVec(new_var_name, 256)
                stack.insert(0, new_var)
                return_node = ReturnStatusNode(new_var_name, new_var, calls[-1])
            ...
            if len(stack) > 1:
                # todo: deal with offset and length, and add return value to graph
                offset = stack.pop(0)
                length = stack.pop(0)
                if opcode == "REVERT":
                    node = TerminalNode(opcode, global_state["pc"])
                    self.x_graph.add_terminal_node(node, path_conditions_and_vars, self.gen.get_path_id())
            else:
                raise ValueError('STACK underflow')
            ...
            for key in global_state["balance"]:
                if convert_result_to_int(key - global_state["receiverAddress"]) == 0:
            ...
            if balance_recipient is None:
                ...
                balance_recipient = BitVec(new_address_value_name, 256)
        ...
        end_time = time.time()
        execution_time = end_time - start_time
        log.mylogger.debug("End executing: " + instr + " symbolic execution time: %.6f s" % execution_time)
        log.mylogger.debug("==============================")
    ...
    def _init_global_state(self, path_conditions_and_vars, global_state):
        ...
        call_data_size = BitVec(call_data_size_name, 256)
        ds_node = InputDataSizeNode(call_data_size_name, call_data_size)
        ...
        path_conditions_and_vars["path_condition"].append(And(constraint0, constraint1, constraint2))
        path_conditions_and_vars["branch_flag"].append(True)
        self.x_graph.add_constraint_node(path_conditions_and_vars,
                                         0,
                                         -1,
        ...
        # symbolic
        global_state["balance"][global_state["senderAddress"]] = (init_is - deposited_value)
        global_state["balance"][global_state["receiverAddress"]] = (init_ia + deposited_value)
            ...
            for old_start in old_keys:
                ...
                if end < old_start:
                    ...
                    if end >= old_end:
                        ...
                        memory[end+1] = (old_end, convert_result(Extract(8*(old_end-old_start+1)-1, 8*(end+1-old_start), to_symbolic(old_value, bits=old_size*8))))
                        memory.pop(old_start)
                elif old_start < start:
                    if old_end < start:
                        continue
                    elif end >= old_end >= start:
                        memory[old_start] = (start-1, convert_result(Extract(8*(start-old_start)-1, 0, to_symbolic(old_value, bits=old_size*8))))
                    else:  # old_end > end
                        memory[old_start] = (start-1, convert_result(Extract(8*(start-old_start)-1, 0, to_symbolic(old_value, bits=old_size*8))))
                        memory[end+1] = (old_end, convert_result(Extract(8*(old_end-old_start+1)-1, 8*(end+1-old_start), to_symbolic(old_value, bits=old_size*8))))
    ...
    # we assume that real index should load value from memory and symbol index should load memory from mem
    # todo debug this function at runtime and implement the unit test,
    #  and simplify for z3 expression of contract and extra
    def load_memory(self, start, params, size):  # size = start - end + 1, e.g. 32
        if is_all_real(start):
            memory = params.memory
            if start in memory:  # memory := (start, (end, value))
                ...
                if end - start == size-1:
                    ...
                    result = Extract(8*size-1, 0, to_symbolic(value, bits=8*(end-start+1)))
                else:
                    result = Concat(to_symbolic(self.load_memory(end+1, params, size-(end-start+1)), bits=(size-(end-start+1))*8), to_symbolic(value, bits=8*(end-start+1)))
            else:
                for x in memory:
                    end = memory[x][0]
                    value = memory[x][1]
                    if x < start <= end:
                        if end - start == size-1:
                            result = Extract(8*(end-x+1)-1, 8*(start-x), to_symbolic(value, bits=8*(end-x+1)))
                        elif end - start > size-1:
                            result = Extract(8*(start+size-x)-1, 8*(start-x), to_symbolic(value, bits=8*(end-x+1)))
                        else:
                            result = Concat(to_symbolic(self.load_memory(end+1, params, size-(end-start+1)), bits=(size-(end-start+1))*8),
                                            Extract(8*(end-x+1)-1, 8*(start-x), to_symbolic(value, bits=8*(end-x+1))))
                        if is_expr(result):
                            assert result.sort() == BitVecSort(8 * size), "load memory is not BitVecSort(256)"

                        return convert_result(result)
                result = BitVecVal(0, 8*size)
        else:
            mem = params.mem
            for x in mem:
                if convert_result_to_int(start - x) == 0 and convert_result_to_int(mem[x][0] - start) == size-1:
                    return mem[x][1]
            else:
                log.mylogger.debug("symbolic index not in mem, create a new memory variable")
...
class Parameter:
    def __init__(self, **kwargs):
        attr_defaults = {
            ...
            "stack": [],
            # all variables located with real type of address and size is stored and loaded by memory,
            # and with one symbolic var in address or size, the value is stored and loaded in mem
            "memory": {},
            "mem": {},
