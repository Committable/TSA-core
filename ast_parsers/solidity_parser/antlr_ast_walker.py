...
class AntlrAstWalker(AstWalkerInterface):
    ...
    def _walk_with_list_of_attrs(self, node, list_of_attributes, nodes):
        if self._check_list_of_attributes(node, list_of_attributes):
            ...
            for x in node:
                if isinstance(node[x], parser.Node):
                    self._walk_with_list_of_attrs(node[x], list_of_attributes,
                                                  nodes)
                elif isinstance(node[x], list):
                    for child in node[x]:
                        if isinstance(child, parser.Node):
    ...
    def _walk_to_json_inner(self, node, depth):
        ...
        json_result[
            'src'] = f'{node["loc"]["start"]["line"]}:{node["loc"]["end"]["line"] + 1}'
        for x in node:
            if isinstance(node[x], parser.Node):
                json_result['children'].append(
                    self._walk_to_json_inner(node[x], depth + 1))
            elif isinstance(node[x], list):
                for child in node[x]:
                    if isinstance(child, parser.Node):
                        json_result['children'].append(
                            self._walk_to_json_inner(child, depth + 1))
        return json_result
