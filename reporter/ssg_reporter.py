...
from graphBuilder.x_graph import ConstraintNode
from abstracts.ssg.ssg_abstract import SsgAbstract


class SsgReporter:
    def __init__(self, output_path):
        self.output_path = output_path
        # one func one cfg_json, but there may be more than on contract for a solidity source file, and more than one
        # function for a contract
        # {"contractName":value}
        self.ssg_json = {}
        self.ssg_graphs = {}
        self.ssg_edge_lists = {}
    ...
    def set_contract_ssg(self, contract_name, xgraph):
        ...
        for key in graphs:
            ...
            for n in list(graph.nodes):
                ...
                self.ssg_json[graph_key]["nodes"].append({"id": str(n),
            ...
            for edge in list(graph.edges):
                ...
                for label in graph.edges[(e, x)]["labels"]:
                    if graph.edges[(e, x)]["label"]:
                        ...
                        graph.edges[(e, x)]["label"] = graph.edges[(e, x)]["label"] + " | " + label

                edge_list.append(node_map[str(e)] + " " + node_map[str(x)] + "\n")
                self.ssg_json[graph_key]["edges"].append(
                    {"source": str(e), "target": str(x), "type": graph.edges[(e, x)]["type"]})
    ...
    def dump_ssg_json(self):
        ...
        with open(self.ssg_json_path, 'w') as output_file:
    ...
    def print_function_ssg_graph(self):
        for func in self.ssg_graphs:
            ...
            for n in list(self.ssg_graphs[func].nodes):
                node = self.ssg_graphs[func].nodes[n]
            ...
            for edge in list(self.ssg_graphs[func].edges):
                ...
                if not g.has_edge(s, t):
                    g.add_edge(s, t,
                               ...
                               label=self.ssg_graphs[func].edges[(s, t)]["label"],
                               style=self.ssg_graphs[func].edges[(s, t)]['style'],
                               color=self.ssg_graphs[func].edges[(s, t)]['color'],
                               )
            ...
            g1.graph_attr['ratio'] = 'fill'
            g1.layout(prog="dot")
            g1.draw(path=os.path.join(self.output_path, func + "_ssg.png"), format='png')
        return
    ...
    def dump_ssg_edge_list(self):
        ...
        with open(self.ssg_edge_lists_path, 'w') as output_file:
            output_file.write("".join(complete))

    def construct_ssg_abstract(self, context):
        ssg_abstract_instance = SsgAbstract()
        ssg_abstract_instance.register_ssg_abstracts(context)
        self.ssg_abstract = ssg_abstract_instance.get_ssg_abstract_json(self.ssg_graphs)

    def dump_ssg_abstract(self):
        self.ssg_abstract_path = os.path.join(self.output_path, "ssg_abstract.json")
        with open(self.ssg_abstract_path, 'w') as output_file:
