...
def update_graph_computed(graph, node_stack, opcode, computed, path_conditions_and_vars, global_state):
    if opcode in one_operand_opcode:
        node_first = node_stack.pop(0)

        operand = [node_first]
        global_state["nodeID"] += 1
        computedNode = ArithNode(opcode, operand, global_state["pc"], computed, global_state["nodeID"])
...
def update_pass(node_stack, opcode, global_state):
    ...
    if OPCODE.push == 1:
        global_state["nodeID"] += 1
        new_selfDefinedNode = SelfDefinedNode(opcode, 0, global_state["nodeID"])
...
def update_graph_msg(graph, node_stack, opcode, global_state):
    global_state["nodeID"] += 1
    if opcode == "CALLER":
        msgNode = MsgDataNode(opcode, global_state["sender_address"], global_state["nodeID"])
    else:
        msgNode = MsgDataNode(opcode, global_state["sender_address"], global_state["nodeID"])
...
def update_graph_block(graph, node_stack, opcode, value, blockNumber, global_state):
    global_state["nodeID"] += 1
    node_block = BlockDataNode(opcode, value, blockNumber, global_state["nodeID"])
...
def update_graph_mload(graph, address, current_miu_i, mem, node_stack, new_var, node_mem, global_state):
    ...
    if isAllReal(address, current_miu_i) and address in mem:
        ...
        global_state["nodeID"] += 1
        temp_new_selfDefinedNode = SelfDefinedNode("MLOAD", 0, global_state["nodeID"])
        node_stack.insert(0, temp_new_selfDefinedNode)
        global_state["nodeID"] += 1
        new_selfDefinedNode = SelfDefinedNode("MLOAD", new_var, global_state["nodeID"])
        if isReal(address):
...
def update_graph_sstore(graph, node_stack, stored_address, global_state, path_conditions_and_vars):
    ...
    if isReal(stored_address):
        if not stored_address in global_state['pos_to_node']:
            global_state["nodeID"] += 1
            node_new_var = StateNode("Ia", stored_address, node_stored_value, stored_address, global_state["nodeID"])
            graph.addNode(node_new_var)
...
def update_jumpi(graph, node_stack, block, flag, branch_expression, global_state):
    ...
    if not isReal(flag):
        ...
        global_state["nodeID"] += 1
        branch_expression_node = ArithNode("!=", operand, 0, branch_expression, global_state["nodeID"])
        global_state["nodeID"] += 1
        negated_branch_expression_node = ArithNode("==", operand, 0, Not(branch_expression), global_state["nodeID"])
...
def update_graph_const(graph, node_stack, value, global_state):
    global_state["nodeID"] += 1
    push_node = ConstNode("", value, global_state["nodeID"])
...
def update_callcode(graph, node_stack, global_state, path_conditions_and_vars):
    ...
    callcode_node = MessageCallNode("CALLCODE", arguments, global_state["pc"],
                                    path_conditions_and_vars["path_condition"], global_state["nodeID"])
...
def update_graph_inputdata(graph, node_stack, new_var, new_var_name, global_state):
    node_position = node_stack.pop(0)
    global_state["nodeID"] += 1
    node_new_var = InputDataNode(new_var_name, new_var, global_state["nodeID"])
    graph.addNode(node_new_var)
    node_stack.insert(0, node_new_var)
