...
from utils import generate_output_dir, change_to_relative, remove_prefix
...
def main():
    ...
    for file in load_test_files("./test_cases/test_fail.json"):
        ...
        log.mylogger.info("-----------------start analysis: %s------------------", os.path.abspath(os.path.join(file["project_dir"],
                                                                                                          file["src_file"])))
        context = Context(time.time(), file["project_dir"], file["src_file"], [], "", "")

        try:
            analyze_evm_from_solidity(file_output_path, context.src_file, context.project_dir, context)
