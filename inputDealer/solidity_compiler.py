    ...
    def __init__(self, project_dir, src_file, root_path, allow_paths, remaps,
        ...
        if compiler_version:
            ...
            if not os.path.exists(self.target):
                ...
                allowed_version = SolidityCompiler.get_compiler_version_from_pragma(
                    self.target)
            ...
            if "prepare" in compilation_cfg:
                for command in compilation_cfg["prepare"]:
                    ...
                    if child.returncode != 0:
                        log.mylogger.error(
                            "run compilation prepare command %s fail", command)
