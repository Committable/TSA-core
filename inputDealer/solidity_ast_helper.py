...
from abstracts.ast.ast_abstract import AstAbstract
...
class AstHelper:
        ...
        if input_type == global_params.LanguageType.SOLIDITY:
            ...
            for x in sources:
                if "legacyAST" in sources[x]:
                    ...
                    raise Exception("unknown ast type for {0}".format(x))
    ...
    def get_ast_json(self, filename, source, context):
        walker = AstWalker(self.ast_type, context.diff)
    ...
    def get_ast_abstract(self, filename, source, context):
        ast_abstract_instance = AstAbstract()
        ast_abstract_instance.register_ast_abstracts(context)
        if filename in self.ast_trees:
            return ast_abstract_instance.get_ast_abstract_json(self.ast_trees[filename], self.ast_type, source)
        else:
    ...
    def extract_contract_definitions(self):
        if self.ast_type == "legacyAST":
            ...
            walker = AstWalker(self.ast_type)
            for k in self.ast_trees:
                ast = self.ast_trees[k]
                nodes = []
