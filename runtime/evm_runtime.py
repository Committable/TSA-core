...
from runtime.basic_block import BasicBlock
...
class EvmRuntime:
    ...
    def __init__(self, context, platform=None, opcodes=None, source_map=None, src_file=None, input_type=None, binary=None):
        self.context = context
        self.opcodes = opcodes  # disassemble file of contract in string like "PUSH1 0x55 PUSH1 0x23 PUSH1 0xB"
        self.source_map = source_map  # SourceMap class of solidity of the contract
        # specified blockchain platform, eg. ethereum, xuper-chain, fisco-bcos, for specific analysis
    ...
    def build_cfg(self):
        if self.input_type == global_params.LanguageType.SOLIDITY:
            ...
            log.mylogger.error("Unknown file type {0}".format(self.input_type))
            ...
            for i in range(start_address, end_address + 1):
                if i in self.instructions:
                    ...
                    if self.source_map.instr_positions:
                        if self.source_map.in_src_file(self.source_map.instr_positions[i]["f"]):
                            ...
                            changed = changed or intersect(self.context.diff, i_lines)
                            for x in i_lines:
                                lines.add(x)
                            if t_start < start:
                                start = t_start
    ...
    def print_cfg(self):
        ...
        g.render(file_name+"_cfg", format='png', directory=global_params.DEST_PATH, view=True)

    def print_visited_cfg(self, visited_edges):
        g = Digraph(name="ControlFlowGraph", format='pdf')
        ...
        for block in self.vertices.values():
            ...
            #     label = self.source_map.source.get_content_from_line(block.lines[0]) + "\n" + "...\n" + \
            ...
            if block_type == "falls_to":
                ...
                g.edge(start, str(block.get_falls_to()), color=color,
        ...
        g.render(os.path.join(global_params.DEST_PATH, "cfg"+self.src_file.split("/")[-1].split(".")[0]+".gv"),
                 view=False)
